...
namespace NavigationBehaviorSpecs
{
    public class LocalAvoidanceJobSpec
    {
        ...
        [Test]
        public void CanDetectAgentCollisions()
        {
            LocalAvoidanceJob job = new LocalAvoidanceJob()
            {
                world_scale = 1f,
            ...
            float3 onTop = new float3(0, 0, 0);
            float3 border = new float3(0, 0, 1f);
            float3 inside = new float3(0, 0, 0.25f);
            float3 outside = new float3(0, 0, 1.25f);

            Assert.AreEqual(true, job.AgentCollision(onTop));
            Assert.AreEqual(true, job.AgentCollision(inside));
            Assert.AreEqual(true, job.AgentCollision(border));
            Assert.AreEqual(false, job.AgentCollision(outside));
        }
    }
}
